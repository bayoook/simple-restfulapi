...
/**
 * Use to handle request for schedule
 */
class ScheduleController {
    ...
    try {
      ...
      if (!methodID || !activityID || !startDate || !endDate) {
        return res.status(400).send(
            responseHelper.fail('error', 'Invalid input'));
      }

      const checkMethodID = await methodModel.getByID(methodID);
      if (checkMethodID.length < 1) {
        return res.status(404).send(responseHelper.fail(
            'error', `Data methods with id ${methodID} not found / deleted`));
      }

      const checkActivityID = await activityModel.getByID(activityID);
      if (checkActivityID.length < 1) {
        return res.status(404).send(
            responseHelper.fail('error',
                `Data activity with id ${activityID} not found / deleted`));
      }

      if (endDate < startDate) {
        return res.status(400).send(
            responseHelper.fail(
                'error', 'Invalid end_date must be bigger then start_date'));
      }

      const newStartDate = converterHelper.epochToTimestamp(startDate);
      const newEndDate = converterHelper.epochToTimestamp(endDate);

      const result = await scheduleModel.insert(
          methodID, activityID, newStartDate, newEndDate);
      if (result.length < 1) {
        return res.status(404).send(
  ...
  async getAll(_, res) {
    try {
      ...
      if (result.length < 1) {
        return res.status(404).send(
            responseHelper.fail('error', `Data schedules not found`));
      }
  ...
  async getByID(req, res) {
    try {
      ...
      if (result.length < 1) {
        return res.status(404).send(
            responseHelper.fail('error',
                `Data schedules with id ${id} not found / deleted`));
      }

      res.status(200).send(responseHelper.success('Success', result[0]));
    } catch (error) {
      console.log(error);
      res.status(500).send(responseHelper
  ...
  async getByTime(req, res) {
    try {
      ...
      if (!checker.isInt(from) || !checker.isInt(to)) {
        return res.status(400).send(
            responseHelper.fail('error', 'Invalid request from / to'));
      }

      if (to < from) {
        return res.status(400).send(responseHelper
            .fail('error', 'Invalid to must be bigger then from'));
      }
      ...
      if (result.length < 1) {
        return res.status(404).send(responseHelper.fail('error',
            `Data schedules with range (${newFrom} -> ${newTo}) not found`));
      }
  ...
  async update(req, res) {
    try {
      ...
      if (checkMethodID.length < 1) {
        return res.status(404).send(
            responseHelper.fail('error',
                `Data methods with id ${methodID} not found / deleted`));
      }
      ...
      if (endDate < startDate) {
        return res.status(400).send(
            responseHelper.fail('error',
                'Invalid end_date must be bigger then start_date'));
      }
      ...
      const result = await scheduleModel.update(
          id, methodID, activityID, newStartDate, newEndDate);
      if (result.length < 1) {
        return res.status(404).send(
            responseHelper.fail('error',
                `Data schedules with id ${id} not found / deleted`));
      ...
      console.log(error);
      res.status(500).send(responseHelper.fail('error',
